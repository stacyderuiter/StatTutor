...
penguins <- read.csv('http://www.calvin.edu/~sld33/data/bandedPenguins.csv')
# reindeer data: ROS (rain on snow) is bad for reindeer, so ROS High is Poor conditions
reindeer <- read.csv('http://www.calvin.edu/~sld33/data/reindeer.csv') %>%
  mutate(WeatherConditions = ifelse(ROS == 'High', 'Poor', 'Good'))

obscene <- read.csv('http://www.calvin.edu/~sld33/data/obscenity.csv') %>%
...
A more concise alternate method we have not yet seen:
...
**We did it!** Nice work. Wouldn't it be nice if we could celebrate the successful completion of the task with an awesome sound effect?

```{r, beepr, exercise=TRUE}
PRD <- do(1000)*diff(prop(~survived == 'lived' | shuffle(tag.type), data=penguins))
beep(sound = 8)
```

There are 11 options (numbers 1-11) for the sound. *You may want to try them all.* This will only work when running code in the console, not when knitting a file. If you want to use it in your own code, make sure to add the line

```{r}
require(beepr)
```

to your setup r code chunk.
...
ss <- data.frame(samp_surv_diff = samp_surv_diff, height=0)
gf_histogram(~ surv_diff , data = PRD2)  %>%
  gf_refine(geom_emoji(emoji = '1f427', mapping = aes(samp_surv_diff, height), data=ss, size=0.07))
```
...
```{r, peng-p-calc, exercise = TRUE, exercise.setup = 'setup-frd', eval=FALSE}
prop1(...)
```
...
Apply the "randomly change signs of differences" strategy to create the randomization distribution.

```{r, robin-rd, exercise = TRUE}

```

```{r, robin-rd-hint-1, eval=TRUE}
RRD <- do(1000)*mean( ~( sample(...)*DiurnalDiff ), data=robins)
```

```{r, robin-rd-hint-2, eval=TRUE}
RRD <- do(1000)*mean( ~( sample(c(-1,1),  size= _____, replace = ______)*DiurnalDiff ),
                      data=robins)
```

```{r, robin-rd-hint-3, eval=TRUE}
RRD <- do(1000)*mean( ~( sample(c(-1,1), size= nrow(robins), replace = TRUE)*DiurnalDiff ),
                      data=robins)
```
...
Use the data at [http://www.calvin.edu/~sld33/data/obscenity.csv](http://www.calvin.edu/~sld33/data/obscenity.csv) and try to carry out tests to verify the findings reported in the abstract. Note: You will have to compare no more than two conditions at a time (for example, sweating at beginning or nowhere) because we haven't yet invented a great sample stat for comparing three groups.

If you want to work here in the tutorial, the data are already loaded for you in the dataset `obscenity`.
...
Mathieu Douhard and colleagues studied the effects of weather on reproductive success in reindeer. For reindeer, frequent rain-on-snow (ROS) is *bad* weather. Using the dataset `reindeer` (already loaded for you in this tutorial, or accessible at [http://www.calvin.edu/~sld33/data/reindeer.csv](http://www.calvin.edu/~sld33/data/reindeer.csv)), try to answer:
