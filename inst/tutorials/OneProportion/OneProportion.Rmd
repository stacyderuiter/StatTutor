...
This module covers **Lock5 Chapter 6.1 - 6.3**. Relevant sections will be noted throughout this tutorial when they are covered.
...
*Text book reference: Lock5 Chapter 6.1, pp. 652 - 653*
...
### Normality of Sampling Dist when $\hat{p} = 0.5$

The code below computes and displays four bootstrap sampling distributions, and overlays a fitted normal distribution on each one.

For each one, **the sample proportion is $\hat{p} = 0.5$.** 
...
```{r, echo = TRUE, fig.show = 'hold', fig.width = 3, fig.height = 2.5}
# note: code written for clarity, not efficiency :)
b1 <- do(1000)*rflip(n = 10,  prob = 0.5)
b2 <- do(1000)*rflip(n = 25,  prob = 0.5)
b3 <- do(1000)*rflip(n = 50,  prob = 0.5)
...
```{r, quiz-med-p}
quiz(question(text = "Which sample sizes result in normal-looking sampling distributions for a proportion (when the sample proportion is 0.5)?",
...
### Normality of Sampling Dist when $\hat{p} = 0.07$

**All** of the sampling distributions we just checked had $\hat{p} = 0.5$. Does varying the sample proportion matter?  

#### Let's repeat the same exploration, but this time with a **very small $\hat{p}$ value of 0.07.**
...
```{r, echo = FALSE, fig.show = 'hold', fig.width = 3, fig.height = 2.5}
# note: code written for clarity, not efficiency :)
b1 <- do(1000)*rflip(n = 10,  prob = 0.07)
b2 <- do(1000)*rflip(n = 25,  prob = 0.07)
b3 <- do(1000)*rflip(n = 50,  prob = 0.07)
...
quiz(question("What does it mean when part of the left tail of the normal curve is 'cut off' on the plot?",
  ...
  question(text = "Which sample sizes result in normal-looking sampling distributions for a proportion (when the sample proportion is 0.0.07)?",
...
### Normality of Sampling Dist when $\hat{p} = 0.88$

**All** of the sampling distributions we just checked had $\hat{p} = 0.07$. Is it only **small** values of $\hat{p}$ that require larger samples sizes to look normal? Or will **big** $\hat{p}$ values have the same problem?  

#### Let's repeat the same exploration, but this time with a **large $\hat{p}$ value of 0.88.** 

*Notice, this $\hat{p}$ is big, but not as extreme as 0.07 -- 0.07 is closer to 0 than 0.88 is to 1.*
...
<details>
  <summary>Show code</summary>
```{r, echo = TRUE, eval = FALSE, fig.show = 'hide', fig.width = 3, fig.height = 2.5}
# note: code written for clarity, not efficiency :)
...
When the sample proportion $\hat{p}$ is in the middle of the range -- around 0.5 -- even a pretty small sample size like $n = 10$ will result in a normal-looking sampling distribution.

But as the sample proportion $\hat{p}$ gets closer to the boundaries of 0 and 1, you need bigger and bigger sample sizes before the sampling distribution looks normal.  This is because proportions can never be less than 0 or more than 1; if $n$ is small and there's a lot of variation from sample to sample, it's limited at the edge of the possible range (so with $p \approx 0$, the sampling distribution can "spread out" toward the right but not to the left).
...
quiz(question("The sampling distribution for a proportion will be skewed (not normal) if the population proportion is close to 0 or 1 and the sample size is small. Which direction is the skew?",
              ...
              answer("Right for small proportions and lefts for large proportions", correct = TRUE),
...
*Text book reference: Lock5 Chapter 6.1*
...
But it tells us more!  It also tells us *what the standard error of the sampling distribution will be*, as a function of the proportion $p$ and the sample size $n$!
...
quiz(question("How do you expect the SE to depend on sample size n?",
     ...
     question("How do you expect the SE to depend on the proportion p?",
              answer("As p increases, SE will increase"),
              answer("As p increases, SE will decrease"),
              answer("There won't be a simple linear relationship between p and SE", correct = TRUE, message = "You're right! In the next section, we'll explore the actual shape of this relationship."),
              allow_retry = TRUE)
)
```
...
You can click to expand the code below to see how it's done, but **your main goal is to see exactly how the SE changes as a function of n**.

<details>
  <summary>Show code</summary>

```{r, echo = TRUE, fig.width = 6.5, fig.height = 2.5, fig.show = 'hold'}
ns <- seq(from=10, by = 50, to = 1000)
SE <- NA*ns
...
gf_point(SE ~ 1/ns, xlab = '1 / Sample Size (n)')
gf_point(SE ~ sqrt(1/ns), xlab = '1 / Square root of\nSample Size')
```

</details>


```{r, echo = FALSE, fig.width = 6.5, fig.height = 2.5, fig.show = 'hold'}
ns <- seq(from=10, by = 50, to = 1000)
SE <- NA*ns
for (i in c(1:length(ns))){
  samp.dist <- do(1000)*rflip(n = ns[i], prob = 0.5)
...
*Text book reference: Lock5 Chapter 6.2*

As long as you know the sample size $n$ and the sample proportion $\hat{p}$, you can now **estimate the SE for a CI without bootstrapping**.
...
One more practice. On the *Factfulness* quiz at the start of term, 21 (of the 37 of us) scored 3 or lower (alas, worse than chimpanzees).  Assuming (generously) that we are a random sample of US college students, find a 90% CI for the proportion US college students who do worse than chimps on this test.
...
We are 90% confident the true proportion chimp-dominated test-takers is between 0.43 and 0.70! (Kind of a lot, but the same is true all over the world.)

## Using your Power Wisely: Tests
*Text book reference: Lock5 Chapter 6.3*
...
[Data from Calvin University](https://calvin.edu/offices-services/enrollment-research/reports/archives/2019/2019-fall-census-day-10-report.pdf#page=1) states that the population proportion of Calvin students who are Christian Reformed Church members is 28.3%. Recall, in our survey, 16 of 37 students were "sunday stalwarts". Let's test whether the proportion stalwarts could be the same as the proportion CRC members.
...
**Conclusion:** The p-value is pretty small - about 0.04. So we reject $H_0$. Our data provide pretty strong evidence *against* the idea that the proportion Stalwarts is the same as the proportion CRC students. (Not surprising - it was a kind of silly idea to begin with!)
...
phat <- 779/n
SE <- sqrt(phat * (1 - phat) / n)
2 * xpnorm(phat, mean = p0, sd = SE, lower.tail = TRUE)
```

The p-value is very small: about $7\times10^{-21}$. We reject $H_0$ and conclude that the proportion first-years is less than 25% (suggesting declining enrollment at Calvin).
...
*Textbook reference: Lock5 Chapter 6.2, page 361*
...
For *additional* practice on one-proportion problems, there's also [a worksheet available for you (with solutions so you can check your work)](https://moodle.calvin.edu/mod/page/view.php?id=1007066).
