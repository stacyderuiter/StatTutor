...
<!-- If desired you can access the original tutorial by running the code below in RStudio.[^original] -->

<!-- [^original]:  Don't do this now, but if you want to see the original -->
<!-- tutorial, you can run this code in R -->
...
But before we do that, take a look at the plot.
  ...
  3. What **improvements** might we make to the plot to test your conjectures?
...
To get R (or any software) to create this plot, 
...
Scatter plots are a common way to investigate the
relationship between two quantitative variables like `births` and `date`,
but they not the only type of plot we could use.
To make a different kind of plot, 
we simply replace `gf_point()` with a different goal.
...
Replace the second `gf_point()` with one of the following and see what happens:
  ...
  * `gf_area()` -- shade the area under `gf_line()`
...
*For the moment, some of these are probably "dangerous" plot types. Why? Because the picture may look pretty, but you may not understand exactly what the function is doing with your dataset to get the picture. And you have to know what is being done to be sure it's appropriate!* 
...
#### Your turn
  ...
  3. Which patterns are easier to see with which types of plots?


## Custom colors and transparency 

Color can be a very important element of a plot.  
...
We can adjust the colors (and several other plot attributes) using the `...` part of our template.
...
Change the color argument so that it maps color to `wday`. 
  ...
  * Don't forget the tilde (`~`) and to remove the quotes.
...
We could investigate the number of births by day of week another way.
Let's make a scatter plot with `wday` on the x-axis and `births` on the 
y-axis.

```{r, q-v-c, exercise = TRUE}
gf_point( births ~ wday, data = Births1978)
gf_violin(births ~ wday, data = Births1978) %>%
gf_sina( births ~ wday, data = Births1978)
```

All those dots kind of run together.  Let's try some improvements:

1. Set `alpha` to a smallish number (start at 0.5 and adjust up or down from there).  This will help show where the dots are piling up.

2. We could move the dots left and right a bit without losing any information.
Do this with `gf_jitter( height = 0)`.  `height = 0` says don't move things up and
down, only left and right.

3. Now try `gf_sina()`.  This slides the points left and right, but moves them
farther when there are more dots and lest far when there are fewer.  That
helps us see the shape of the data.

4. Now try `gf_violin()`.  This is like the outline of the dots in `gf_sina()`.
(You might like to overlay these plots to compare.)

5. Some of you may have seen boxplots before. `gf_boxplot()` will create those.

Which of these plots do you like best for this data set?  Why?

## Exagerating the differences?

You may have noticed that these plots don't stretch down to 0 on the y-axis.
By default, R tries to fit everything onto the plot with relatively little 
extra space.  This makes our data easier to see (because it takes up more
space).  But sometimes this distorts our view of the data.  We can tell
R exactly what range to use using `gf_lims()`. Here is an example:

```{r gf-lims, exerciese = TRUE}
gf_violin(births ~ wday, data = Births1978) %>%
  gf_sina(births ~ wday, data = Births1978) %>%
  gf_lims(y = c(0, NA))
```
...
* The `c()` function may seem a bit strange.  Think of c = combine.  We are 
combining two values into one range of values.

* `NA` is R's way of saying "missing".  By not specifying the top of the range,
we let R pick the top for us, we just want to pick the lower limit.  (Change `NA` to some number to see how the plot changes.)

* Does your impression of how different the weekdays and weekends are 
change when you change the limits?

## Plots with just one variable

The most interesting plots have multiple variables, but sometimes it 
is useful to look at just one variable.  In this case, we only need one 
slot in our formula.  For example, to make a histogram we use

```{r histogram, exericse = TRUE}
gf_histogram( ~ births, data = Births1978) 
```

We are telling R that we want `births` on the x-axis and we expect R to 
compute what goes on the y-axis for us.

### Change it up!

1. Change the widths of the bins by setting `binwidth` to the width of a bin
or `bins` to the number of bins.

2. Change `gf_histogram()` to `gf_density()` or `gf_dens()` to get a 
"smooth" version of the histogram called a density plot.  (Density plots
are like half of a violin plot.)

3. Just like you can adjust the binwidth in a histogram, you can
adjust the smoothness of a density plot.  The default value is 
`adjust = 1`.  Set `adjust` to 0.5 or 2.0 to see how the density plot 
changes.

3. `gf_dhistogram()` makes a **density histogram**.  A density
histogram is rescaled so that it's **total area is 1**.  Density plots
also have a total area of 1.  Overlay a density histogram and a 
density plot.  Which works better, `gf_dens()` or `gf_density()`?

4. What happens if you map color or fill to `wday` in a density plot?  Give
it a try!

5. Overlay a histogram with `gf_freqpoly()`.  Be sure to set the binwidth
to the same value for each.  How do the plots compare?  When might you
prefer a frequeny polygon to a histogram?

## One Template to Rule A Lot

We have made a lot of plots, all with one basic template.  
Not quite as good as Frodo's ring, but pretty powerful.

Let's catch our breath and make sure  we still see the big picture.
The basic structure of all of these commands is the same:
...
* goal: says what kind of plot to make (and starts `gf_` every time).
* `y ~ x` (or sometimes just `~ x`) says which variable to put
  on the x-axis and which variable to put on the y-axis.
* `data = ` tells R which dataset to use (where to find the variables).
* `...` is a comma-separated list of additional instructions in 
  the form `attribute = value`.  We can use these extras to set or map
  other properties, like color or fill or shape, or to set values for 
  things like the binwidth of a histogram or the smoothness of a density
  plot.
* We can layer plots on top of each other using `%>%`.
  
* Once you get the basic structure, the commands are all basically the 
same!

* And it works for numbers as well as plots!!!  Let's give that a try.

### Computing Numerical Summaries

All we need to do is change the name of our goal from the name of a 
plot to the name of a numerical summary.

```{r number-summary, exercise = TRUE}
gf_violin(births ~ wday, data = Births1978)
     mean(births ~ wday, data = Births1978)
gf_histogram(~ births, data = Births1978)
        mean(~ births, data = Births1978)
```

### Your Turn!

```{r number-summary-your-turn, exercise = TRUE}
mean(births ~ wday, data = Births1978)
```

Add additional lines to the code chunk above to do the following.

1. Compute the maximum number of births for each day of the week (`max()`).

2. Compute the minimum number of births for each day of the week (`min()`).

3. Compute the median number of births for each day of the week (`median()`)


### Multi-taksing

Hmm. What if we want several numbers all at once?  Give this a try:

```{r df-stats, exercise = TRUE}
df_stats(births ~ wday, data = Births1978, mean, median)
```

1. What happens if you delete `mean` and `median` (and the unneeded commas) 
in the example above?

2. What happens if you add `min` and `max` in the example above?

3. What happens if you use `range` instead of `min` and `max`?

Note: The result of `df_stats()` is a **data frame**.  That's R's name 
for rectangular data.  That explains the "df".  "stats", of course,
stands for "statistics", numbers we compute from our data.

So now we can make both graphical and numerical summaries of our data
with one easy template:
...
It would probably be a good idea to master that template. We are going to use it
**a lot**.
...
To find out more about the data sets use `?HELPrct`, `?KidsFeet`, or  `?penguins`.
...
as there are peole in your group.
