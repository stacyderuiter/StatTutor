...
will use a **density histogram** (`gf_dhistogram()`).[^dhistogram]
...
The values of `fig.width` and `fig.height` are expected to be given in inches, by default. Just add these values to the chunk header like this:

```{r echo = FALSE, results = 'asis', comment = NA}
cat("```{r fig.width = 6, fig.height = 4}")
```
...
gf_boxplot(BPSysAve ~ TVHrsDay, data = NHANES)
      mean(BPSysAve ~ TVHrsDay, data = NHANES, na.rm = TRUE)
```

Why `na.rm = TRUE`?  Edit the code above to remove that part and 
see what happens.  To R's way of thinking, if some of the values are 
missing, you can't know what the sample mean is, so it reports NA.
`na.rm = TRUE` says "Please remove all missing values first, then
compute the mean for me.  Thanks!"
