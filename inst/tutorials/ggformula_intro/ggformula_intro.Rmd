...
The formula interface can be used for numerical summaries as well.
For example, to compute the mean number of births, we simply replace
`gf_histogram()` with `mean()` to get a numerical summary rather than a graphical one.

```{r, numerical01, exercise = TRUE}
...
This same thing works for several other functions as well.  Experiment with
`median()`, `max()`, `range()`, `sum()`, etc.

```{r, numerical02, exercise = TRUE}
mean(~ births, data = Births1978)
```

### Summarizing by groups

Even better, we can do our summaries by groups.  For example, we can get the mean
number of births for each day of the week.  Once again, we just replace an appropriate
plotting command with the numerical summary we want.

```{r, numerical03, exercise = TRUE}
...
Edit the code below to do the following.
 ...
 1. Try some other numerical summaries in place of `mean()`.
 
 2. Use `Births` and compute summaries by year.
 
```{r, numerical04, exercise = TRUE}
mean( births ~ wday, data = Births1978)
```

### Multi-tasking

Sometimes it is nice to compute multiple numerical summaries at once.
`df_stats()` is designed for this.  (This starts `df_` because the result is a 
data frame rather than a graph.)  We simply include a comma separated 
list of summary functions in addition to our formula and data.

```{r, df_stats, exercise = TRUE}
df_stats( births ~ wday, data = Births1978, min, max)
```

### Exercises

 1. Try adding some additional numerical summaries to the example above.
 
 2. What happens if you name the summaries? (Example: `lo = min, hi = max`.)
 
 3. What happens if you use a summary like `range` that computes more than one number?
 (What if you give it a name?)
 
 4. What happens if you don't include any numerical summaries?  (Trust me, it's a good thing.)
 
 5. Now try adding a grouping variable.
 
 6. Now try using `Births` and grouping by both `year` and `wday`.
 
```{r, df_stats_exercise, exercise = TRUE}
df_stats( births ~ wday, data = Births1978, min, max)
```

### df_stats() and plots

One nice things about `df_stats()` is that it returns a data frame with 
a convenient structure.
This means we can use the results from `df_stats()` in our plots.

```{r, df_stats_plotting, exercise = TRUE}
Births1978_summary <- df_stats(births ~ wday, data = Births1978, mean, median)
gf_violin( births ~ wday, data = Births1978) %>%
  gf_point( mean_births ~ wday, color = "navy", alpha = 0.5, data = Births1978_summary) %>%
  gf_point( median_births ~ wday, color = "red", shape = 1, data = Births1978_summary)
```
...
```{r, df_stats_pipe, exercise = TRUE}
Births1978 %>% df_stats(births ~ wday, mean, median)
```
...
Make some plots and numerical summaries to explore one or more of these data sets.
