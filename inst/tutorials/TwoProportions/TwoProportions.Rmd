...
CI <- stat + c(-1,1) * 1.96 * 0.06421489 # use SE from above here
CI
```

**Solution:** We are 95% confident that the true difference in proportion brown-eyed islanders (South minus North Island) is between -0.09 and 0.16.  This means it is plausible that the proportion brown-eyed people is the same on both islands, since 0 (no difference) is in the CI.
...
quiz(question("What null hypothesis will we test?",
           ...
           answer("$H_0: \\hat{p} = 0.036$", message = "Oops. we *know* that the sample difference in proportions is 0.036 -- that is just a fact, not a testable hypothesis."),
           allow_retry = TRUE,
...
Wait a second.  If we are **assuming $H_0$ is true**, then suddenly (and weirdly!), $\hat{p}_S - \hat{p}_N$ is *no longer* our best estimate of the true difference in proportions $p_S - p_N$.
