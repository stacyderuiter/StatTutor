...
From this exploration of the predictions, it's pretty clear that our model is not great at predicting sex from fWHR.  Could we measure how well it's doing, or test whether fWHR is a "good" predictor?  We can consider using ANOVA for this.

## Prediction plots in R
All the ways that we have made predictions so far have been kind of a LOT of R code and a lot of work for us. Can we hand all that work over to R and get  a pretty picture out? YES!

In other words, we want a simpler way to create  *prediction plots* showing model values. Ideally, we want a graph that **includes uncertainty** (that is, confidence intervals on the predicted values). And one that  will work if and when we include *more* than just a single predictor variable.  We can use the `ggeffects::ggpredict()` function to do ALL this, just like we did for linear regression models fitted with `lm()`.

```{r}
library(ggeffects)
ggpredict(bonobo_logit, 
          terms = c("fWHR")) |>
  plot()
```

The only downside there is the y axis label: It's labelled "Probability of Sex" but which sex? Edit below to improve the label:

```{r, relabel-y, exercise = TRUE}
library(ggeffects)
ggpredict(bonobo_logit, 
          terms = c("fWHR")) |>
  plot()
...
ggpredict(bonobo_logit, 
          terms = c("fWHR")) |>
  plot() |>
  gf_labs(y = "Predicted probability of being male")
```

The code above will work for any model; you just  need to customize the inputs: the name of *your* fitted model instead of `bonobo_logit`; the name of *your* predictor(s) of interest instead of "fWHR"; and an axis label that makes sense for *your* model.
...
We saw earlier how to make these prediction plots using `ggeffects::ggpredict() |> plot()`.
